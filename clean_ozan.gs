...
function updateTargetDocument_ozan() {
    ...
    const targetDocumentId = "1NqhIzcclsxSmMm1uax7e16nzHoUtTIK-4pXJFlpVspY";
    const targetSheetName = "liveTraining";
    ...
    for (let i = 1; i < targetData.length; i++) {
        ...
        let stdName = findCompanyNameByOrgAndCountry(externalData, orgName, country, externalCompanyNameColIdx).trim();
        if (stdName) {
            targetSheet.getRange(i + 1, targetUpdateColIdx + 1).setValue(stdName);
        } else {
            // Update company_std based on matching email pattern
            stdName = findCompanyNameInExternal(externalData, domain, country).trim();
            targetSheet.getRange(i + 1, targetUpdateColIdx + 1).setValue(stdName);
        }
    }
